...
export const sendEmail = async ({ to, subject, html }: SendEmailOptions) => {
    try {
        ...
        if (process.env.MOCK_EMAIL === 'true' || process.env.CI === 'true' || process.env.NODE_ENV === 'test') {
            console.log(`[MOCK EMAIL] To: ${to} | Subject: ${subject}`);
            await HookSystem.dispatch('core.email.sent', { to, subject, messageId: 'mock-id-' + Date.now() });
