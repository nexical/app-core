...
export function parseQuery<T = any>(searchParams: URLSearchParams, options: FilterOptions): { where: T; take: number; skip: number; orderBy?: any } {
    const where: any = {};
    ...
    for (const [key, value] of searchParams.entries()) {
        ...
        switch (operator) {
            case 'eq':
                where[field] = fieldType === 'boolean' ? parseBoolean(value) : (fieldType === 'number' ? Number(value) : value);
                break;
            case 'ne':
                where[field]['not'] = fieldType === 'boolean' ? parseBoolean(value) : (fieldType === 'number' ? Number(value) : value);
                break;
            case 'contains':
