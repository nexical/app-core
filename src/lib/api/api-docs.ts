...
export type ApiHandler<T = any> = (context: APIContext, actor: ApiActor) => Promise<Response | T> | Response | T;
...
export function defineApi<T>(handler: ApiHandler<T>, docs: OpenAPIRouteDocs = {}): ApiRouteHandler {
    const wrappedHandler: APIRoute = async (context) => {
        ...
        try {
            ...
            if (lowerMessage.includes("unauthorized") || lowerMessage.includes("forbidden") || lowerMessage.includes("denied") || lowerMessage.includes("not a member")) {
                status = 403;
            } else if ((err as any)?.name === 'ZodError' || lowerMessage.includes("invalid") || lowerMessage.includes("already exists") || lowerMessage.includes("exists") || lowerMessage.includes("mismatch")) {
                status = 400;
 ...
 */
export async function generateDocs(module: { path: string, name: string }, actor?: ApiActor): Promise<Record<string, any>> {
