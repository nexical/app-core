...
describe('AuditApiCommand', () => {
    ...
    describe('auditModule logic', () => {
        ...
        it('should handle audit exceptions gracefully', async () => {
            ...
            vi.spyOn(fs, 'readFileSync').mockImplementation(() => { throw new Error('Abort'); });
        ...
        it('should perform full audit with code checks (including role objects and api.yaml)', async () => {
            ...
            vi.spyOn(fs, 'readdirSync').mockImplementation((p: any) => {
                const pathStr = p.toString();
                if (pathStr.includes('modules') && pathStr.includes('src/roles')) return ['admin.ts'] as any;
            ...
            vi.spyOn(fs, 'readFileSync').mockImplementation((p: any) => {
                const pathStr = p.toString();
                if (pathStr.includes('models.yaml')) return 'models: { User: { role: "admin", api: true } }';
            ...
            vi.spyOn(YAML, 'parse').mockImplementation((content: string | any) => {
                ...
                if (contentStr.includes('models:')) return { models: { User: { role: "admin", api: true } } };
                if (contentStr.includes('path:')) return { User: [{ path: "/me", method: "GET" }] };
                return {};
            });

            vi.spyOn(ModelParser, 'parse').mockReturnValue({
                models: [{ name: 'User', db: false, api: true, fields: { id: 'String' } }], enums: []
            } as any);
        ...
        it('should report semantic errors in models.yaml', async () => {
            ...
            vi.spyOn(fs, 'readFileSync').mockReturnValue('models: { User: { role: { a: "r" }, fields: { name: "Unknown" } } }');
            vi.spyOn(YAML, 'parse').mockReturnValue({ models: { User: { role: { a: "r" }, fields: { name: "Unknown" } } } });
        ...
        it('should validate db-enabled models and hit service/api builder branches', async () => {
            vi.spyOn(fs, 'existsSync').mockReturnValue(true);
            vi.spyOn(fs, 'readdirSync').mockReturnValue([]);
            vi.spyOn(fs, 'readFileSync').mockReturnValue('models: { Post: { db: true, api: true, fields: { title: "String" } } }');
            vi.spyOn(YAML, 'parse').mockReturnValue({ models: { Post: { db: true, api: true, fields: { title: "String" } } } });

            vi.spyOn(ModelParser, 'parse').mockReturnValue({
                models: [{ name: 'Post', db: true, api: true, fields: { title: 'String' } }], enums: []
            } as any);
