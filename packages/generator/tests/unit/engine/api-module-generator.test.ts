...
describe('ApiModuleGenerator', () => {
    ...
    it('should orchestration the full generation process', async () => {
        const mockModels = [
            { name: 'User', api: true, db: true, fields: { email: { type: 'String' } }, test: { actor: 'User' } }
        ];
        ...
        vi.mocked(ServiceBuilder).mockImplementation(function () { return { ensure: mockEnsure } as any; } as any);
        vi.mocked(ApiBuilder).mockImplementation(function () { return { ensure: mockEnsure } as any; } as any);
    ...
    it('should handle virtual resources from api.yaml', async () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue('Root: [{ method: "ping", path: "/ping", verb: "GET" }]');
        ...
        await generator.run();
        expect(ApiBuilder).toHaveBeenCalledWith(expect.objectContaining({ name: 'Root' }), expect.any(Array), expect.any(String), 'custom', expect.any(Array));
    });
    ...
    it('should hit diverse branches (non-db models, normalization, paths)', async () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue('External: [{ method: "sync", path: "sync", verb: "" }]');
