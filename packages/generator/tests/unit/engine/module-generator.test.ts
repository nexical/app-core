...
class TestGenerator extends ModuleGenerator {
    async run() { }
    public testGetOrCreateFile(p: string) { return this.getOrCreateFile(p); }
    public testCleanup(d: string, p: RegExp) { return this.cleanup(d, p); }
    public async testSaveAll() { return this.saveAll(); }
}
