...
describe('FactoryBuilder', () => {
    ...
    it('should handle relations and special fields', () => {
        ...
        const initText = sourceFile.getVariableStatement('factories')?.getDeclarations()[0].getInitializer()?.getText();
        expect(initText).toContain('username: `username_${index}_${crypto.randomUUID().split(\'-\')[0]}`');
