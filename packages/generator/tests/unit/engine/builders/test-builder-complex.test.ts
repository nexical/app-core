...
describe('TestBuilder Complex Scenarios', () => {
    ...
    const relationModel: ModelDef = {
        ...
        fields: {
            ...
            category: { type: 'Category', isRequired: true, isList: false, api: false, isRelation: true, attributes: ['@relation(fields: [categoryId])'] },
            // Actor relation via custom field
            assigneeId: { type: 'Int', isRequired: false, isList: false, api: true, attributes: [] },
            assignee: { type: 'User', isRequired: false, isList: false, api: false, isRelation: true, attributes: ['@relation(fields: [assigneeId])'] }
        },
        test: { actor: 'User' }
    };
    ...
    it('should handle update logic with complex and reserved fields', () => {
        ...
        expect(text).toContain('expect(updated?.scheduledAt.toISOString()).toBe(updatePayload.scheduledAt)');
    });
    ...
    it('should identify actor relation via foreign key attribute', () => {
        ...
        expect(text).toContain('assigneeId: (actor ? actor.id : undefined)');
    });
