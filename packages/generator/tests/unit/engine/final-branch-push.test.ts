...
describe('Final Branch Push - ApiBuilder Edge Cases', () => {
    it('should handle custom route with path parameters and DTOs', async () => {
        ...
        const builder = new ApiBuilder(mockModel as any, [mockModel] as any, 'user-api', 'custom', mockRoutes as any);
    ...
    it('should filter private fields in relations', async () => {
        const mockModel = {
            name: 'Post', api: true, db: true, fields: {
                id: { type: 'String' },
        ...
        const allModels = [
            mockModel,
            { name: 'User', fields: { id: { type: 'String' }, password: { type: 'String', private: true } } }
        ];
    ...
    it('should handle anonymous role in custom routes', async () => {
        const mockModel = { name: 'User', api: true, db: true, fields: { id: { type: 'String' } } };
        const mockRoutes = [
            { method: 'ping', path: 'ping', verb: 'POST', role: 'anonymous' }
        ];
