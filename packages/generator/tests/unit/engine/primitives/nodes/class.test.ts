...
describe('ClassPrimitive', () => {
    ...
    it('should update extends clause', () => {
        const testProject = createTestProject();
        const sourceFile = testProject.createSourceFile('test.ts', 'class TestClass extends OldBase {}');
        const primitive = new ClassPrimitive({
            name: 'TestClass',
            extends: 'NewBase'
        });
    ...
    it('should update implements clauses (additive)', () => {
        ...
        primitive.ensure(sourceFile);
        const impls = sourceFile.getClass('TestClass')?.getImplements().map(i => i.getText());
