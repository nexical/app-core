...
describe('ImportPrimitive', () => {
    ...
    it('should normalize module specifiers', () => {
        ...
        // @ts-ignore
        expect(primitive.normalizeModuleSpecifier('@modules/user-api/src/sdk/types.ts')).toBe('@modules/user-api/src/sdk');
    });
    ...
    it('should deduplicate imports from the same module', () => {
        ...
        const primitive = new ImportPrimitive({ moduleSpecifier: '@/lib/core/utils', namedImports: ['A', 'B'] });

        const node = sourceFile.getImportDeclarations()[0];
        primitive.update(node);

        expect(sourceFile.getImportDeclarations()).toHaveLength(1);
        const named = sourceFile.getImportDeclarations()[0].getNamedImports().map(ni => ni.getText());
    ...
    it('should remove duplicate symbols from similar paths', () => {
        ...
        const primitive = new ImportPrimitive({ moduleSpecifier: '@modules/target', namedImports: ['Dupe'] });

        const node = sourceFile.addImportDeclaration({ moduleSpecifier: '@modules/target', namedImports: ['Dupe'] });
        primitive.update(node);

        const otherImport = sourceFile.getImportDeclaration(d => d.getModuleSpecifierValue() === '@modules/other');
    ...
    it('should enforce type-only', () => {
        ...
        const primitive = new ImportPrimitive({ moduleSpecifier: './mod', isTypeOnly: true, namedImports: ['A'] });
        primitive.update(node);
        expect(node.isTypeOnly()).toBe(true);
    });

    it('should manage named imports (add/remove)', () => {
        const node = sourceFile.addImportDeclaration({ moduleSpecifier: './mod', namedImports: ['A', 'C'] });
    ...
    it('should update module specifier if not normalized', () => {
        const node = sourceFile.addImportDeclaration({ moduleSpecifier: './mod.ts', namedImports: ['A'] });
