...
vi.mock('commander', () => ({
    program: mockProgram,
    Command: vi.fn().mockImplementation(function () { return mockProgram; })
}));
...
vi.mock('fs', () => ({
    ...
    readdirSync: vi.fn(),
    statSync: vi.fn(),
    fileURLToPath: vi.fn().mockReturnValue('/home/adrian/Projects/nexical/app-core/packages/generator/src/cli.ts')
}));

// Mock dynamic commands
vi.mock('@nexical/generator/commands/gen/api', () => ({
    GenApiCommand: class { getCommand() { return { name: 'gen' }; } }
}));
vi.mock('@nexical/generator/commands/audit/api', () => ({
    AuditApiCommand: class { getCommand() { return { name: 'audit' }; } }
}));
