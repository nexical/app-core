...
describe('PlatformParser', () => {
    beforeEach(() => {
        vi.resetAllMocks();
        vi.mocked(PlatformDefinitionSchema.safeParse).mockReturnValue({ success: true, data: {} } as any);
    });
    ...
    it('should parse valid YAML and validate schema', () => {
        ...
        vi.mocked(PlatformDefinitionSchema.safeParse).mockReturnValue({ success: true, data: mockYaml } as any);
    ...
    it('should throw error on validation failure', () => {
        ...
        vi.mocked(PlatformDefinitionSchema.safeParse).mockReturnValue({ success: false, error: { issues: [] } } as any);
