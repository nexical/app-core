...
export class ApiModuleGenerator extends ModuleGenerator {
    async run(): Promise<void> {
        ...
        const customRoutes: Record<string, CustomRoute[]> = fs.existsSync(apiYamlPath) ? parse(fs.readFileSync(apiYamlPath, "utf-8")) : {};
        ...
        for (const model of models) {
            ...
            const kebabName = name.replace(/([a-z])([A-Z])/g, "$1-$2").replace(/[\s_]+/g, '-').toLowerCase();
            ...
            if (model.api && !model.extended) {
                ...
                for (const [routePath, routes] of Object.entries(groupedRoutes)) {
                    ...
                    for (const route of routes) {
                        // Action Stub
                        const actionBase = route.action || `${route.method.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()}-${kebabName}`;
                        ...
                        new ActionBuilder(actionName, route.input || 'any', route.output || 'any').ensure(actionFile);
                    }
                }
                ...
                if (model.db && !model.extended) {
                    ...
                    for (const op of ops) {
                        ...
                        const testFile = this.getOrCreateFile(`tests/integration/api/generated/${kebabName}/${op}.test.ts`);
        ...
        for (const [entityName, routes] of Object.entries(customRoutes)) {
            console.log("Checking virtual model:", entityName, "Processed:", processedModels.has(entityName));
            ...
            for (const route of routes) {
                // Robust normalization of Verb
                if (!route.verb && ['GET', 'POST', 'PUT', 'DELETE', 'PATCH'].includes(route.method?.toUpperCase())) {
            ...
            for (const [fileName, routes] of Object.entries(groupedVirtualRoutes)) {
                ...
                new ApiBuilder(virtualModel, [...models, ...virtualModels], this.moduleName, 'custom', routes).ensure(apiFile);

                for (const route of routes) {
                    // Action
                    const actionBase = route.action || `${route.method.replace(/([a-z])([A-Z])/g, "$1-$2").toLowerCase()}-${kebabEntity}`;
                    const actionPath = `src/actions/${actionBase}.ts`;
                    const actionFile = this.getOrCreateFile(actionPath);
                    const actionName = route.action
                        ? route.action.split('-').map(s => s.charAt(0).toUpperCase() + s.slice(1)).join('') + 'Action'
