...
export class Reconciler {
    static reconcile(sourceFile: NodeContainer, definition: FileDefinition): void {
        const filePath = 'getFilePath' in sourceFile ? (sourceFile as SourceFile).getFilePath() : 'namespace';
        ...
        try {
            ...
            definition.classes?.forEach(classDef => {
                ...
                properties?.forEach(propDef =>
                    new PropertyPrimitive(propDef).ensure(classNode)
                );
                ...
                accessors?.forEach(accDef =>
                    new AccessorPrimitive(accDef).ensure(classNode)
                );

                // Recursive: Handle Methods
                methods?.forEach(methodDef =>
                    new MethodPrimitive(methodDef).ensure(classNode)
                );
            });
            ...
            definition.enums?.forEach(enumDef =>
                new EnumPrimitive(enumDef).ensure(sourceFile)
            );

            // 5. Handle Functions
            definition.functions?.forEach(funcDef =>
                new FunctionPrimitive(funcDef).ensure(sourceFile)
            );

            // 6. Handle Types
            definition.types?.forEach(typeDef =>
                new TypePrimitive(typeDef).ensure(sourceFile)
            );

            // 7. Handle Variables
            definition.variables?.forEach(varDef =>
                new VariablePrimitive(varDef).ensure(sourceFile)
            );

            // 8. Handle Modules (Namespaces)
            definition.modules?.forEach(modDef =>
                new ModulePrimitive(modDef).ensure(sourceFile)
            );
            ...
            if ('statements' in definition && Array.isArray((definition as any).statements)) {
                ...
                if ('addStatements' in sourceFile) {
                    ...
                    const uniqueStmts = stmts.filter(stmt => !existingText.includes(Normalizer.normalize(stmt)));
    ...
    static validate(sourceFile: NodeContainer, definition: FileDefinition): import("./primitives/contracts.js").ValidationResult {
        const issues: string[] = [];
