...
export class ExportPrimitive extends BasePrimitive<ExportDeclaration, ExportConfig> {
    ...
    find(parent: SourceFile) {
        return parent.getExportDeclaration(decl =>
    ...
    update(node: ExportDeclaration) {
        ...
        if (this.config.exportClause === '*') {
            ...
            const missingExports = this.config.exportClause.filter(ne => !normalizedExisting.includes(ne));
    ...
    validate(node: ExportDeclaration): import("../contracts.js").ValidationResult {
        ...
        if (Array.isArray(this.config.exportClause)) {
            ...
            const missingExports = this.config.exportClause.filter(ne => !normalizedExisting.includes(ne));
            if (missingExports.length > 0) {
                issues.push(`Export '${this.config.moduleSpecifier}' missing named exports: ${missingExports.join(', ')}`);
            }
        ...
        // Check Type Only
        if (this.config.isTypeOnly !== undefined && node.isTypeOnly() !== this.config.isTypeOnly) {
            issues.push(`Export '${this.config.moduleSpecifier}' type-only mismatch. Expected: ${this.config.isTypeOnly}, Found: ${node.isTypeOnly()}`);
        }
