...
export class ImportPrimitive extends BasePrimitive<ImportDeclaration, ImportConfig> {
    ...
    update(node: ImportDeclaration) {
            ...
            if (isTargetAliased === isExistingAliased) {
                ...
                existingNamed.forEach(ni => {
                    ...
                    if (targetSymbols.includes(sym)) {
                        console.log(`[ImportPrimitive] Removing duplicate symbol '${sym}' from ${normalizedExisting} (moving to ${normalizedTarget})`);
                        ni.remove();
                    }
                });
        ...
        if (this.config.namedImports) {
            ...
            const missingImports = this.config.namedImports.filter(ni => !normalizedExisting.includes(ni));
        ...
        if (!node.getDefaultImport() && node.getNamedImports().length === 0 && !node.getNamespaceImport()) {
            node.remove();
        }
    }
    validate(node: ImportDeclaration): import("../contracts.js").ValidationResult {
        ...
        if (normalizedExisting !== normalizedTarget) {
            issues.push(`Import module specifier mismatch. Expected: ${normalizedTarget} (normalized), Found: ${normalizedExisting}`);
        }
        ...
        if (this.config.defaultImport) {
            ...
            if (!defaultImport || defaultImport.getText() !== this.config.defaultImport) {
                issues.push(`Import '${this.config.moduleSpecifier}' default import mismatch. Expected: ${this.config.defaultImport}, Found: ${defaultImport?.getText() ?? 'none'}`);
            }
        }
        ...
        if (this.config.namedImports) {
            ...
            if (missingImports.length > 0) {
                issues.push(`Import '${this.config.moduleSpecifier}' missing named imports: ${missingImports.join(', ')}`);
            }
        }

        // Check Type Only
        if (this.config.isTypeOnly !== undefined && node.isTypeOnly() !== this.config.isTypeOnly) {
            issues.push(`Import '${this.config.moduleSpecifier}' type-only mismatch. Expected: ${this.config.isTypeOnly}, Found: ${node.isTypeOnly()}`);
        }
