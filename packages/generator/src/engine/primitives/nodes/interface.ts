...
export class InterfacePrimitive extends BasePrimitive<InterfaceDeclaration, InterfaceConfig> {
    ...
    update(node: InterfaceDeclaration) {
        ...
        if (this.config.properties) {
            this.config.properties.forEach(propConfig => {
                ...
                if (prop) {
                    ...
                    if (propConfig.optional !== undefined && prop.hasQuestionToken() !== propConfig.optional) {
        ...
        if (this.config.extends) {
            ...
            const needsUpdate = this.config.extends.length !== currentExtends.length ||
        ...
        if (this.config.properties) {
            this.config.properties.forEach(p => {
                ...
                if (!prop) {
                    ...
                    if (curType !== neuType) {
                        issues.push(`Interface '${this.config.name}' property '${p.name}' type mismatch. Expected: ${p.type}, Found: ${curTypeRaw}`);
                    }
                }
            });
        }
