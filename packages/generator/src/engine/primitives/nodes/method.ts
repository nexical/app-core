...
export class MethodPrimitive extends BasePrimitive<MethodDeclaration, MethodConfig> {
    ...
    update(node: MethodDeclaration) {
        ...
        if (currentParams.length !== newParams.length) {
            ...
            for (let i = 0; i < currentParams.length; i++) {
                ...
                if (cur.getName() !== neu.name) { paramsChanged = true; break; }
                ...
                if (Normalizer.normalizeType(curType) !== Normalizer.normalizeType(neuType)) {
                    console.log(`[MethodPrimitive] Param mismatch for ${this.config.name}: '${curType}' != '${neuType}'`);
                    paramsChanged = true; break;
                }
    ...
    validate(node: MethodDeclaration): ValidationResult {
        ...
        if (structure.isAsync !== undefined && method.isAsync() !== structure.isAsync) {
            issues.push(`Method '${this.config.name}' async modifier mismatch. Expected: ${structure.isAsync}, Found: ${method.isAsync()}`);
        }

        if (structure.isStatic !== undefined && method.isStatic() !== structure.isStatic) {
            issues.push(`Method '${this.config.name}' static modifier mismatch. Expected: ${structure.isStatic}, Found: ${method.isStatic()}`);
        }

        const returnTypeNode = method.getReturnTypeNode();
        const currentReturnTypeRaw = returnTypeNode?.getText();
        const currentReturnType = currentReturnTypeRaw ? Normalizer.normalizeType(currentReturnTypeRaw) : undefined;

        if (typeof structure.returnType === 'string' && currentReturnType !== Normalizer.normalizeType(structure.returnType)) {
            issues.push(`Method '${this.config.name}' return type mismatch. Expected: ${structure.returnType}, Found: ${currentReturnTypeRaw || 'implicit/void'}`);
        }
        ...
        if (currentParams.length !== newParams.length) {
            issues.push(`Method '${this.config.name}' parameter count mismatch. Expected: ${newParams.length}, Found: ${currentParams.length}`);
        } else {
            for (let i = 0; i < currentParams.length; i++) {
                const cur = currentParams[i];
                const neu = newParams[i];
                ...
                if (cur.getName() !== neu.name) {
                    issues.push(`Method '${this.config.name}' parameter ${i} name mismatch. Expected: ${neu.name}, Found: ${cur.getName()}`);
                }
                ...
                if (curType !== Normalizer.normalizeType(neuType)) {
                    issues.push(`Method '${this.config.name}' parameter '${neu.name}' type mismatch. Expected: ${neuType}, Found: ${curTypeRaw}`);
                }
            }
        }
