...
export class DecoratorPrimitive extends BasePrimitive<Decorator, DecoratorConfig> {
    ...
    update(node: Decorator) {
        ...
        const isArgsDrift = currentArgs.length !== targetArgs.length ||
    ...
    validate(node: Decorator): ValidationResult {
        ...
        if (currentArgs.length !== targetArgs.length) {
            issues.push(`Decorator '@${this.config.name}' argument count mismatch. Expected: ${targetArgs.length}, Found: ${currentArgs.length}`);
        } else {
            // Check content
            currentArgs.forEach((arg, i) => {
                if (arg !== targetArgs[i]) {
                    issues.push(`Decorator '@${this.config.name}' argument ${i} mismatch. Expected: ${targetArgs[i]}, Found: ${arg}`);
                }
            });
        }
