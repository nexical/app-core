...
export class FunctionPrimitive extends BasePrimitive<FunctionDeclaration, FunctionConfig> {
    ...
    update(node: FunctionDeclaration) {
        ...
        if (structure.returnType && Normalizer.normalizeType(node.getReturnType().getText()) !== Normalizer.normalizeType(structure.returnType as string)) {
        ...
        if (structure.parameters) {
            ...
            structure.parameters.forEach((paramStruct, index) => {
                ...
                if (existingParam) {
                    ...
                    if (paramStruct.type && Normalizer.normalizeType(existingParam.getTypeNode()?.getText() || "") !== Normalizer.normalizeType(paramStruct.type as string)) {
    ...
    validate(node: FunctionDeclaration): ValidationResult {
        ...
        if (structure.isAsync !== undefined && node.isAsync() !== structure.isAsync) {
            issues.push(`Function '${this.config.name}' async modifier mismatch. Expected: ${structure.isAsync}, Found: ${node.isAsync()}`);
        }

        if (structure.returnType && Normalizer.normalizeType(node.getReturnType().getText()) !== Normalizer.normalizeType(structure.returnType as string)) {
            issues.push(`Function '${this.config.name}' return type mismatch. Expected: ${structure.returnType}, Found: ${node.getReturnType().getText()}`);
        }
        ...
        for (const stmtConfig of this.config.statements) {
            ...
            if (stmtConfig.kind === 'variable') {
                ...
                const existingVar = node.getVariableStatement(v =>
                    v.getDeclarationList().getDeclarations().some(d => d.getName() === varName)
                );
                ...
                const existingReturn = node.getStatements().find(s => s.getKindName() === 'ReturnStatement');
                if (existingReturn) {
                    if (!stmtConfig.isDefault) {
                ...
                const existingIf = node.getStatements().find(s =>
                    s.getKindName() === 'IfStatement' && s.getText().includes(condition)
                );
                ...
                const existingThrow = node.getStatements().find(s =>
                    s.getKindName() === 'ThrowStatement' && s.getText().includes(expr)
                );
                if (existingThrow) {
                    if (!stmtConfig.isDefault) {
