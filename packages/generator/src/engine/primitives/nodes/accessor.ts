...
export class AccessorPrimitive extends BasePrimitive<AccessorDeclaration, AccessorConfig> {
    ...
    update(node: AccessorDeclaration) {
        if (this.config.kind === 'get' && node instanceof GetAccessorDeclaration) {
            ...
            if (this.config.returnType && Normalizer.normalizeType(node.getReturnType().getText()) !== Normalizer.normalizeType(this.config.returnType)) {
            ...
            if (this.config.parameters && this.config.parameters.length > 0) {
                ...
                if (param && this.config.parameters[0].type && Normalizer.normalizeType(param.getTypeNode()?.getText() || "") !== Normalizer.normalizeType(this.config.parameters[0].type)) {
