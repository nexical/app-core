    ...
    protected getSchema(node?: any): FileDefinition {
        ...
        if (node && 'getClass' in node) {
            ...
            if (cls) {
                ...
                if (method) {
                    console.log(`[ActionBuilder] Found existing method 'run' (static: ${method.isStatic()}) in ${this.actionName}`);
        ...
        const namedImports = [this.inputType, this.outputType]
            ...
            .filter(t => {
                ...
                return !['string', 'number', 'boolean', 'any', 'void', 'unknown', 'never', 'undefined', 'object', 'null', 'date'].includes(normalized);
            });
        ...
        if (namedImports.length > 0) {
            ...
            if (uniqueImports.length > 0) {
                imports.push({ moduleSpecifier: "../sdk/types", namedImports: uniqueImports, isTypeOnly: true });
            }
        }
