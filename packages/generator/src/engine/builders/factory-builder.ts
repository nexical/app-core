...
import { type ModelDef, type FileDefinition, type VariableConfig, type FunctionConfig } from "../types.js";
...
export class FactoryBuilder extends BaseBuilder {
    ...
    protected getSchema(node?: any): FileDefinition {
        ...
        for (const model of this.models) {
            ...
            for (const [fieldName, field] of Object.entries(model.fields)) {
                const isIdWithDefault = fieldName === 'id' && field.attributes?.some(a => a.startsWith('@default'));
                if ((fieldName === 'id' && isIdWithDefault) || fieldName === 'createdAt' || fieldName === 'updatedAt') continue;
                ...
                if (type === 'string') {
                    ...
                    } else if (fieldName === 'username' || fieldName === 'token' || field.attributes?.some(a => a.includes('@unique'))) {
