    ...
    private async auditModule(name: string, checkSchemaOnly: boolean): Promise<string[]> {
        ...
        try {
            ...
            const validTypes = new Set([
                'String', 'Boolean', 'Int', 'BigInt', 'Float', 'Decimal', 'DateTime', 'Json', 'Bytes', // Prisma Scalars
                'unknown', 'any', 'void' // Special Types
            ]);
                ...
                for (const [modelName, modelDef] of Object.entries<any>(parsedModels.models)) {
                    ...
                    if (modelDef.fields) {
                        for (const [fieldName, fieldDef] of Object.entries<any>(modelDef.fields)) {
                            ...
                            if (!validTypes.has(fieldType)) {
                                report(chalk.red(`[Semantic] Model '${modelName}.${fieldName}' has unknown type '${fieldType}'`));
                            }
                        }
                    }
                    ...
                    if (modelDef.role) {
                        ...
                        rolesToCheck.forEach((r) => {
                            if (!validRoles.has(r)) {
                                report(chalk.red(`[Semantic] Model '${modelName}' has unknown role '${r}'. Valid: ${Array.from(validRoles).join(', ')}`));
                            }
                        });
                    }
                }
                ...
                try {
                    ...
                    for (const [entityName, routes] of Object.entries(parsedApi)) {
                        routes.forEach((route, idx) => {
                            ...
                            // Check Output Type
                            if (route.output) {
                                const outputType = route.output.endsWith('[]') ? route.output.slice(0, -2) : route.output;
                                if (!validTypes.has(outputType)) {
                                    report(chalk.red(`[Semantic] ${label} has unknown output type '${route.output}'`));
                                }
                            }
                            ...
                            if (route.role) {
                                if (!validRoles.has(route.role)) {
                                    report(chalk.red(`[Semantic] ${label} has unknown role '${route.role}'. Valid: ${Array.from(validRoles).join(', ')}`));
                                }
                            }
                        });
                    }
            ...
            const customRoutes: Record<string, CustomRoute[]> = fs.existsSync(apiPath) ? YAML.parse(fs.readFileSync(apiPath, "utf-8")) : {};
            ...
            for (const model of models) {
                ...
                // Services
                if (model.db) {
                    validate(new ServiceBuilder(model), getFile(`src/services/${kebabName}-service.ts`), `${entityName}Service`);
                }

                if (model.api) {
                    if (model.db) {
                        validate(new ApiBuilder(model, models, name, 'collection'), getFile(`src/pages/api/${kebabName}/index.ts`), `${entityName}API List`);
                        validate(new ApiBuilder(model, models, name, 'individual'), getFile(`src/pages/api/${kebabName}/[id].ts`), `${entityName}API Detail`);
                    }
                    ...
                    for (const route of modelRoutes) {
                        ...
                        validate(new ApiBuilder(model, models, name, 'custom', [route]), getFile(`src/pages/api/${kebabName}/${routePath}.ts`), `${entityName}API ${routePath}`);
                        ...
                        validate(new ActionBuilder(actionName, route.input || 'any', route.output || 'any'), getFile(actionPath), `${entityName}Action`);
                    }

                    // SDK
                    validate(new SdkBuilder(model, modelRoutes), getFile(`src/sdk/${kebabName}-sdk.ts`), `${entityName}SDK`);

                    // Tests
                    if (model.db) {
                        const ops: ('create' | 'list' | 'get' | 'update' | 'delete')[] = ['create', 'list', 'get', 'update', 'delete'];
                        for (const op of ops) {
                            validate(new TestBuilder(model, name, op), getFile(`tests/integration/api/${kebabName}/${op}.test.ts`), `${entityName}Test ${op}`);
                        }
                    }
                }
            }
            ...
            for (const [entityName, routes] of Object.entries(customRoutes)) {
                ...
                for (const route of routes) {
                    ...
                    validate(new ApiBuilder(virtualModel, [...models, ...virtualModels], name, 'custom', [route]), getFile(apiPath), `${entityName}API ${fileName}`);
            ...
            // Global Files
            validate(new SdkIndexBuilder([...models, ...virtualModels], name), getFile("src/sdk/index.ts"), "SDK Index");
