...
function getTargets(): Target[] {
    ...
    return entries
        ...
        .map(dirent => {
            ...
            return {
                name: skillName.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') + " Skill",
...
function stageInstructor(target: Target, canonFile: string, reportFile: string, feedbackFile?: string) {
...
async function main() {
    ...
    console.log(`Total Skills: ${allTargets.length} | Completed: ${completedTargets.length} | Remaining: ${targets.length}`);
        ...
        try {
            ...
            while (attempts < MAX_RETRIES) {
                ...
                if (attempts < MAX_RETRIES) {
                    console.log(`\u26A0\ufe0f  Gauntlet Failed (Attempt ${attempts}/${MAX_RETRIES}). Looping back to Instructor...`);
