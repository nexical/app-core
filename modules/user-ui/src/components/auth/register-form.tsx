...
const registerSchema = (t: (key: string) => string) => z.object({
    ...
    username: z.string().min(3, t("user.auth.register.validation.username_min")).regex(/^[a-zA-Z0-9_]+$/, t("user.auth.register.validation.username_regex")),
    ...
    confirmPassword: z.string().min(8, t("user.auth.register.validation.confirm_password_required")),
...
export function RegisterForm() {
    ...
    const onSubmit = async (values: RegisterFormValues) => {
        ...
        try {
            ...
            const apiError = (err as ApiError).body?.error || (err as ApiError).body?.message || (err as ApiError).message;
            if (apiError && (apiError.includes("invited users only") || apiError.includes("registration_restricted"))) {
            ...
            <Card className="auth-success-card">
                ...
                <CardContent className="auth-form-container">
                    ...
                    <div className="auth-success-message-container">
                        <a href={`/login?callbackUrl=${registeredTeamId ? `/teams/${registeredTeamId}` : (inviteValues?.token ? `/invite/${inviteValues.token}` : '/')}`}>
                            <Button variant="outline" className="auth-return-button" data-testid="proceed-to-login-btn">
        ...
        <Card className="auth-card">
            ...
            <CardContent>
                <form onSubmit={form.handleSubmit(onSubmit)} className="auth-form-container" method="POST">
                    ...
                    {serverError && (
                        <div id="register-error" className="auth-form-message auth-form-message-error" data-testid="register-error">
                            {serverError}
                        </div>
                    )}
                    <div className="auth-form-group">
                        ...
                        {form.formState.errors.name && (
                            <p className="auth-form-input-error" data-testid="field-error-name">{form.formState.errors.name.message}</p>
                        )}
                    </div>
                    ...
                    <div className="auth-form-group">
                        ...
                        {form.formState.errors.username && (
                            <p className="auth-form-input-error" data-testid="field-error-username">{form.formState.errors.username.message}</p>
                        )}
                    </div>
                    ...
                    <div className="auth-form-group">
                        ...
                        <Input
                            id="email"
                            type="email"
                            placeholder={t("user.auth.register.email_placeholder", { domain: config.PUBLIC_PRIMARY_DOMAIN })}
                            className="auth-input"
                        ...
                        {form.formState.errors.email && (
                            <p className="auth-form-input-error" data-testid="field-error-email">{form.formState.errors.email.message}</p>
                        )}
                    </div>
                    ...
                    <div className="auth-form-group">
                        ...
                        {form.formState.errors.password && (
                            <p className="auth-form-input-error" data-testid="field-error-password">{form.formState.errors.password.message}</p>
                        )}
                    </div>

                    <div className="auth-form-group">
                        <Label htmlFor="confirmPassword">{t("user.auth.register.confirm_password_label")}</Label>
                        <PasswordInput
                            id="confirmPassword"
                            placeholder="********"
                            className="auth-input"
                        ...
                        {form.formState.errors.confirmPassword && (
                            <p className="auth-form-input-error" data-testid="field-error-confirmPassword">{form.formState.errors.confirmPassword.message}</p>
                        )}
                    </div>
                    ...
                    >
                        {isSubmitting ? t("user.auth.register.submitting_button") : t("user.auth.register.submit_button")}
                    </Button>
