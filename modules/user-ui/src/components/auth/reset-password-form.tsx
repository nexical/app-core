...
const resetSchema = (t: (key: string) => string) => z.object({
    ...
    confirmPassword: z.string().min(8, t("user.auth.reset_password.validation.confirm_password_required")),
...
export function ResetPasswordForm({ token, isValid }: { token: string, isValid: boolean }) {
    ...
    const onSubmit = async (values: ResetFormValues) => {
        ...
        try {
            ...
            const errorMessage = (err as ApiError).body?.error || (err as ApiError).message || t("user.auth.reset_password.error.unexpected");
    ...
    if (!isValid) {
        return (
            <Card className="auth-card">
                <CardHeader>
                    <CardTitle className="auth-card-title">{t("user.auth.reset_password.expired_title")}</CardTitle>
                </CardHeader>
                <CardContent className="auth-link-expired-content">
                    <p className="auth-link-expired-text" data-testid="reset-password-expired-text">{t("user.auth.reset_password.expired_message")}</p>
                    <a href="/forgot-password" className="auth-link-expired-action" data-testid="reset-password-new-link">{t("user.auth.reset_password.request_new_link")}</a>
                </CardContent>
            </Card>
        )
    }
        ...
        <Card className="auth-card">
            ...
            <CardContent>
                <form onSubmit={form.handleSubmit(onSubmit)} className="auth-form-container" method="POST">
                    {serverError && (
                        <div className="auth-form-message auth-form-message-error" data-testid="reset-password-error">
                            {serverError}
                        </div>
                    )}
                    ...
                    <div className="auth-form-group">
                        <Label htmlFor="confirmPassword">{t("user.auth.reset_password.confirm_password_label")}</Label>
                        <PasswordInput
                            id="confirmPassword"
                            placeholder="********"
                            className="auth-input"
                        ...
                        {form.formState.errors.confirmPassword && (
                            <p className="auth-form-input-error">{form.formState.errors.confirmPassword.message}</p>
                        )}
                    </div>
                    ...
                    >
                        {isSubmitting ? t("user.auth.reset_password.submitting_button") : t("user.auth.reset_password.submit_button")}
                    </Button>
                </form>
            </CardContent>
        </Card>
