...
const passwordSchema = (t: (key: string) => string) => z.object({
...
export function PasswordForm({ user }: PasswordFormProps) {
    ...
    const onSubmit = async (values: PasswordFormValues) => {
        ...
        try {
            ...
            console.error(error);
            const errorMessage = (error as ApiError).body?.error || (error as ApiError).message || t("user.settings.password.error_message");
            ...
            <CardContent>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <Button type="submit" disabled={isSubmitting}>
                        {isSubmitting ? t("user.auth.login.submitting_button") : t("user.settings.password.save_button")}
                    </Button>
                </form>
            </CardContent>
        </Card>
