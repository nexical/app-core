...
const _test = describe("User API - Update", () => {
  ...
  describe("PUT /api/user/[id]", () => {
    it("should update user", async () => {
      ...
      const updated = await Factory.prisma.user.findUnique({
        where: { id: target.id },
      });
      expect(updated?.username).toBe(updatePayload.username);
      expect(updated?.email).toBe(updatePayload.email);
      expect(updated?.passwordUpdatedAt.toISOString()).toBe(
        updatePayload.passwordUpdatedAt,
      ); // Compare as ISO strings
      expect(updated?.emailVerified.toISOString()).toBe(
