            ...
            await sendEmail({
                ...
                html: await EmailRegistry.render('user:verify-email', {
                    ...
                    strings: {
                        ...
                        copyright: translate("user.email.layout.copyright", { year: new Date().getFullYear().toString(), siteName: config.PUBLIC_SITE_NAME })
                    }
                })
            });
        });
        ...
        HookSystem.on('invitation.created', async (event: any) => {
            ...
            await sendEmail({
                ...
                html: await EmailRegistry.render('user:invite', {
                    ...
                    workspaceName,
                    strings: {
                        preview: translate("user.email.invite.preview", { workspaceName, siteName: config.PUBLIC_SITE_NAME }),
                        greeting: translate("user.email.invite.greeting"),
                        body: translate("user.email.invite.body", { inviterName, workspaceName, role: invitation.role }), // Use invitation.role directly as string
