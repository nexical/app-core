...
export class __Entity__OpsService {
     ...
     */
    public static async processOperation(id: string, actor?: ApiActor): Promise<ServiceResponse<any>> {
        try {
            // 1. Hook: Filter (Pre-execution validation/modification)
            // Allows other modules to modify input or block the action.
            const { id: filteredId } = await HookSystem.filter('__entity__.beforeProcess', { id }, { actor });
