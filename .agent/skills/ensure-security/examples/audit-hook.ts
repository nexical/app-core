...
export class SecurityAuditHooks {
  ...
  static init() {
    ...
    HookSystem.on('auth.login_failed', async (event: { email: string; ip: string; reason: string }) => {
    ...
    HookSystem.on('role.assigned', async (event: { userId: string; role: string; assignedBy: string }) => {
