...
const create__Name__Schema = (t: (key: string) => string) => z.object({
...
export function __Name__Form({ onSuccess }: __Name__FormProps) {
    ...
    const onSubmit = async (data: __Name__FormValues) => {
        ...
        try {
            ...
            setServerError(apiError.body?.error || apiError.message || t('__module__.errors.generic_failure'));
        }
    };
    ...
    return (
        ...
        >
            {serverError && (
                <div className="p-3 bg-destructive/10 border border-destructive/20 text-destructive text-sm rounded-lg" data-testid="form-server-error">
                    {serverError}
                </div>
            )}
