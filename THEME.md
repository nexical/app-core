...
The Nexical styling system is a **Semantic Abstraction Layer** built on top of Tailwind CSS. It decouples *design intent* (Semantics) from *visual implementation* (Primitives).
...
### The Cascade
1.  `theme.base.css`: Defines the *structure* (utilities) and *default values* (variables).
2.  `modules/{theme}/theme.css`: Overrides *values* (variables) to change the look.
