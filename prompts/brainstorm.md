...
3. **Spot Weaknesses:** Look for code that handles errors genericly (e.g., `catch (e)`) and ask the user what the *specific* expected user experience should be.
...
*Wait for the user's feedback. iterate on the logic until the user says "Proceed" or "Generate".*
...
* **Future Proofing:** If the code is missing logic (e.g., missing specific error toasts), document the *Ideal State* so the developers know what to build.
