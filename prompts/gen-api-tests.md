...
*   **Implementation agnostic:** Write tests based on the *interface* defined in the CASES file.
...
*   *Expansion:* If you identify a critical edge case missing from `{{ root_path }}tests/integration/CASES.md` file (e.g., "Concurrent Modification"), **ADD** a test case for it and mark it with a comment `// ADDED: Edge case not in original spec`.
