...
1.  **Visual vs. Structural:**
    *   **TARGET (Visual):** Colors (`background`, `color`, `border-color`), Shadows (`box-shadow`), Radius (`border-radius`), Borders (`border`), Spacing (`padding`, `margin`, `gap`), Typography (`font-size`, `font-weight` *if* it maps to a text utility).
...
**STEP 1: Discovery**
Scan the `{{ root_path }}styles.css` file. List every CSS class that contains hardcoded *visual* properties.
