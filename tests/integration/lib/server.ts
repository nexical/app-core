...
export class ServerManager {
    ...
    public async start(): Promise<void> {
        ...
        this.serverProcess = spawn('node', [astroPath, 'dev', '--port', String(port), '--host', TEST_HOST], {
            env: spawnEnv,
            cwd: process.cwd(),
            stdio: 'pipe',
            detached: true,
    ...
    private async waitForServer(): Promise<void> {
        ...
        while (Date.now() - startTime < timeoutMs) {
            try {
                ...
                if (process.env.DEBUG) console.log(`[Polling] ${this.currentUrl}/api/status -> ${res.status}`);
                if (res.ok) {
                    return;
                }
            } catch (e) {
                if (process.env.DEBUG) console.log(`[Polling] ${this.currentUrl}/api/status -> Connection refused/error`);
            }
