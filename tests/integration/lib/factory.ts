...
export class DataFactory {
    ...
    async clean() {
        try {
            ...
            models.forEach(model => {
                model.fields.forEach(field => {
                    if (field.kind === 'object' && field.relationFromFields && field.relationFromFields.length > 0) {
                        if (field.type !== model.name) { // Ignore self-references
            ...
            const deleteActions = sortedModelNames.map(modelName => {
                // @ts-ignore
    ...
    async create<T = any>(model: keyof typeof db, data: any = {}): Promise<T> {
        ...
        if (!(model in this._client)) {
            const available = Object.keys(this._client).filter(k => !k.startsWith('$') && !k.startsWith('_'));
            throw new Error(`Model '${String(model)}' does not exist on Prisma Client. Available: ${available.join(', ')}`);
        }
        ...
        if (builder) {
            ...
            console.warn(`[Factory] No builder found for model '${String(model)}'. Available: ${Array.from(this.registry.keys()).join(', ')}`);
