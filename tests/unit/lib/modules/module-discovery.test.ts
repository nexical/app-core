...
describe('ModuleDiscovery', () => {
    ...
    it('should apply defaults if config is missing or fails to load', async () => {
        vi.mocked(fs.existsSync).mockImplementation((path: any) => typeof path === 'string' && path.endsWith('modules')); // Only modules dir exists
    ...
    it('should fallback to default phase and order for unknown module types', async () => {
        ...
        mockJiti.import.mockImplementation(async (path) => {
            ...
            if (path.includes('another-unknown')) return { default: { type: 'unknown' as any, order: 2 } };
            return { default: {} };
        });
    ...
    it('should sort unknown phases to the default position (20)', async () => {
        ...
        mockJiti.import.mockImplementation(async (path) => {
            if (path.includes('mod-unknown')) return { default: { type: 'something-else' as any, order: 1 } };
    ...
    it('should use default export from module.config.mjs if available', async () => {
        vi.mocked(fs.existsSync).mockImplementation((p: any) => p.includes('modules') || p.includes('module.config.mjs'));
