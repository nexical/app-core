...
import { ModuleDiscovery, type ModulePhase } from '@/lib/modules/module-discovery';
...
describe('ModuleDiscovery', () => {
  ...
  it('should fallback to default phase and order for unknown module types', async () => {
    ...
    mockJiti.import.mockImplementation(async (path: string) => {
      if (path.includes('unknown-type'))
        return { default: { type: 'unknown' as ModulePhase, order: 1 } };
      if (path.includes('another-unknown'))
        return { default: { type: 'unknown' as ModulePhase, order: 2 } };
      return { default: {} };
    });
  ...
  it('should sort unknown phases to the default position (20)', async () => {
    ...
    mockJiti.import.mockImplementation(async (path: string) => {
      if (path.includes('mod-unknown'))
        return { default: { type: 'something-else' as ModulePhase, order: 1 } };
