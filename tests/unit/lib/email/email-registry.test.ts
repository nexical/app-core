...
describe('EmailRegistry', () => {
    ...
    it('should render a template to HTML', async () => {
        const MockTemplate = ({ name }: { name: string }) => React.createElement('div', null, `Hello ${name}`);
    ...
    it('should throw error if rendering missing template', async () => {
        await expect(EmailRegistry.render('unknown', {})).rejects.toThrow('Email template not found: unknown');
    });
});
