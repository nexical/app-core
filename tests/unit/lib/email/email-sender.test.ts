...
describe('email-sender', () => {
    ...
    it('should use mock email in test environment by default', async () => {
        ...
        expect(mockDispatch).toHaveBeenCalledWith('core.email.sent', expect.objectContaining({ to: 'test@example.com' }));
    });
    ...
    it('should use nodemailer when not in test/mock environment', async () => {
        ...
        const result = await sendEmail({ to: 'real@example.com', subject: 'SMTP', html: '<i>SMTP</i>' });

        expect(result.success).toBe(true);
        expect(result.messageId).toBe('123');
        expect(mockDispatch).toHaveBeenCalledWith('core.email.sent', expect.objectContaining({ messageId: '123' }));
    });
    ...
    it('should handle SMTP errors gracefully', async () => {
        ...
        expect(mockDispatch).toHaveBeenCalledWith('core.email.failed', expect.objectContaining({ error: 'SMTP connection failed' }));
    });
});
