...
describe('Core Logger', () => {
    ...
    it('should log info messages as JSON', () => {
        ...
        expect(console.log).toHaveBeenCalledWith(JSON.stringify({ level: 'info', message: 'test message', foo: 'bar' }));
    });
    ...
    it('should log error messages with non-Error as JSON', () => {
        ...
        expect(console.error).toHaveBeenCalledWith(JSON.stringify({ level: 'error', message: 'error message', error: 'string error' }));
    });

    it('should log warn messages as JSON', () => {
        Logger.warn('warn message', { type: 'alert' });
        expect(console.warn).toHaveBeenCalledWith(JSON.stringify({ level: 'warn', message: 'warn message', type: 'alert' }));
    });
    ...
    it('should log debug messages in non-production environment', () => {
        ...
        expect(console.debug).toHaveBeenCalledWith(JSON.stringify({ level: 'debug', message: 'debug message', context: 'test' }));
    });
});
